...
# Thumbnail - 썸네일
# ResizeToFill(가로, 세로) - 원본 비율 유지, 넘어가는 부분 잘라냄
# ResizeToFit(가로, 세로) - 원본 비율 무시, 대신 모든 이미지가 보이도록 압축 
...
class Student(models.Model):
    ...
    image = ProcessedImageField(
        blank = True,
        upload_to ='students/image', # 업로드 위치
    ...
    # blank = True -> 비어있는 값(ex. '')이 들어올 수 있음.
    # null = True -> None(NULL)이 들어 올수 있음
